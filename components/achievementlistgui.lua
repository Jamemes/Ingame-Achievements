if type(tag_print) ~= "function" then
...
if type(table.count) ~= "function" then
...
if type(table.list_reverse) ~= "function" then
...
if type(table.filter_list) ~= "function" then
...
dofile(ModPath .. string.lower("components/achievementdetailgui.lua"))
dofile(ModPath .. string.lower("components/achievementrecentlistgui.lua"))

local function texture_replace(original, replacer)
...
local tiny_font = tweak_data.menu.pd2_small_font
...
function AchievementButton:init(parent, id, info, icon, small_icon, on_trigger)
	...
	local replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/star_button", "guis/textures/pd2/crimenet_marker_host")
	self._button = self._select_panel:bitmap({
		texture = texture,
		texture_rect = replaced and {0, 0, 31, 31},
		w = replaced and 21,
		h = replaced and 21,
		color = self.N_COLOR
	})
	...
	replaced = nil
	if small_icon == "guis/dlcs/trk/textures/pd2/eye_small" then
		replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/eye_small", "guis/textures/pd2/icon_detection")
	else
		replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/star_small", "guis/textures/pd2/mission_briefing/difficulty_icons")
		replaced = replaced and "star"
	end
	...
	self._button_icon = self._select_panel:bitmap({
		...
		h = replaced and 16,
		w = replaced and 16,
		texture_rect = replaced == "star" and {1, 33, 32, 32},
		texture = texture,
		color = self.S_COLOR,
		layer = self:layer() + 1
	})
...
AchievementListItem.NT_SD_COLOR = Color(255, 145, 145, 145) / 25
...
function AchievementListItem:init(parent, data, owner)
	...
	if not awarded then
		...
		local replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/lock", "guis/textures/pd2/blackmarket/important_lock")
		local lock = self._panel:bitmap({
			texture = texture,
			w = replaced and 16 or bitmap:w(),
			h = replaced and 16 or bitmap:h()
		})
	...
	if visual.unlock_id then
		...
		local replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/locked", "guis/textures/pd2/hud_bag")
		if awarded then
			replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/unlocked", "guis/textures/pd2/pd2_waypoints")
		end
		local safe = self:bitmap({
			w = 32,
			h = 32,
			texture_rect = replaced and {0, 0, 32, 32},
			texture = texture,
			x = left
		})
...
function AchievementListGui:init(ws, fullscreen_ws, node)
	...
	local replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/eye", "guis/textures/pd2/icon_detection")
	...
	local replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/star", "guis/textures/pd2/mission_briefing/difficulty_icons")
	self._tracked_icon = b_placer:add_left(self._main_panel:fit_bitmap({
		texture = texture,
		texture_rect = replaced and {1, 33, 32, 32},
		h = self._forced_text:h() - 2,
		color = Color.white
	}))
	...
	for k, data in pairs(tweak_data.achievement.visual) do
		if managers.achievment.achievments[k] then
...
function AchievementListGui:update_detail()
	...
	if extra_text then
		placer:add_bottom(canvas:fine_text({
			keep_w = true,
			text = managers.localization:text(extra_text),
			font = small_font,
			font_size = small_font_size,
...
function AchievementListGui:add_key_press(k, func, requirement)
	if type(requirement) ~= "nil" and not requirement then
	else
		if not self[k .. "_pressed"] and Input:keyboard():down(Idstring(k)) then
			func()
			self[k .. "_pressed"] = true
		elseif self[k .. "_pressed"] and not Input:keyboard():down(Idstring(k)) then
			self[k .. "_pressed"] = nil
		end
	end
end
...
function AchievementListGui:update(...)
	...
	if not self._search_focus then
		self:add_key_press("t", callback(self, self, "_on_toggle_tracked"))
		self:add_key_press("space", callback(self, self, "_on_preview"), self._main_panel.enabled)
		self:add_key_press("y", callback(self, self, "_on_force"))
		self:add_key_press("u", callback(self, self, "_on_toggle_unlocked"))
		self:add_key_press("tab", callback(self, self, "_show_tracked"), not self._view_tracked)
		self:add_key_press("tab", callback(self, self, "_show_all"), self._view_tracked)
		self:add_key_press("f", callback(self, self, "open_filter_popup"), not Global.achievements_filter_on_filters_done)
		self:add_key_press("backspace", callback(self, self, "_clear_filters"))
	end
...
function AchievementListGui:_on_filters_done()
	Global.achievements_filter_on_filters_done = nil
	self:remove_blur()
	self:_redo_filter_and_sort()
...
function AchievementListGui:open_filter_popup()
	...
	Global.achievements_filter_on_filters_done = {
...
Hooks:Add("MenuComponentManagerInitialize", "AchievementListGui.MenuComponentManagerInitialize", function(component)
		component._generated = component._generated or {}
	...
	component._active_components.achievement_list = component:create_component_callback("AchievementListGui", "achievement_list")
end)