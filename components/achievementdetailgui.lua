local function texture_replace(original, replacer)
	if DB:has(Idstring("texture"), Idstring(original)) then
		return not DB:has(Idstring("texture"), Idstring(original)), original
	else
		return not DB:has(Idstring("texture"), Idstring(original)), replacer
	end
end
...
function AchievementDetailGui:init(parent, achievement_data_or_id, back_callback)
	...
	for _, tag in pairs(self._visual.tags) do
		...
		local str = managers.localization:text("menu_achievements_" .. category) .. ": " .. managers.localization:text(id or "")
	...
	local replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/circle_inside", "guis/textures/pd2/hud_progress_active")
	...
	local replaced, texture = texture_replace("guis/dlcs/trk/textures/pd2/circle_outside", "guis/textures/pd2/hud_progress_active")
	...
	if managers.network.account:signin_state() == "signed in" and SystemInfo.distribution then
